...
function buildBook(raw: RawBook, usedSlugs: Set<string>): Book {
...
const books: Book[] = (booksData as RawBook[]).map((raw) => buildBook(raw, usedSlugs));
...
export function getMemberRatings(name: string): MemberRating[] {
  ...
  for (const book of books) {
    ...
    if (entry && entry.score !== null) ratings.push({ book, score: entry.score });
  }
